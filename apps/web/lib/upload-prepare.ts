...
/** Multi-plate extraction shares one MAX_TRIANGLES budget, so its combined STL
 * output is one maximum STL plus at most one extra 84-byte header per plate. */
...
export interface PreparedUploadModel {
  ...
  fileHash: string;
  sizeBytes: number;
  /** False only when the original temp file can be renamed into place. */
  derived: boolean;
