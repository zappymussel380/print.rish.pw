...
| Quotation access | New 256-bit capabilities are stored only as SHA-256 verifiers and expire after 30 days. The browser transports the initial token in a URL fragment, redeems it through a same-origin rate-limited POST, clears the fragment, and uses a per-quotation `HttpOnly` cookie. Confirmation/PDF responses are private, `no-store`, `no-referrer`, and `noindex`. Query parameters never grant quotation access. |
| Upload storage | One file is streamed to an exclusively created private `0600` temporary file, then ownership of that UUID name and its worst-case expansion reservation transfers to a bounded ingest ticket. The worker re-derives the path, opens it without following symlinks, and verifies regular-file size and SHA-256. Original STL/OBJ/raw-AMF bytes are copied from that descriptor to an exclusively created unpredictable UUID path; every 3MF and zipped AMF is instead persisted as canonical binary STL using the same exclusive creation. Controls include hard file/session/model limits, per-IP byte budgets, an atomic 25-ticket admission bound, capacity reservations retained through terminal processing, and a final free-space recheck preserving a 2 GiB reserve by default. |
| Geometry parsing | Text models are limited to 32 MiB, XML to 8 MiB, one million vertices, four million triangles, 100,000 XML elements, depth 128, 1,024 ZIP entries, 64 MiB total relevant extraction, 20 plates, and bounded 3MF component expansion. DTDs, duplicate relevant 3MF entries, oversized tags/settings, invalid consumed indices, unknown units, and decompression overruns are rejected. App-selected archive geometry is normalized to millimetres and binary STL, so the original attacker-controlled ZIP namespace never reaches Orca; downstream STL parsing is still a separate parser boundary. Parsing runs in the backend-only worker under BullMQ local and global concurrency one, not on the public web event loop. |
| Ingest and slice queues | Ingest tickets are fresh UUIDs, session-owned, rate-limited when polled, retained about one hour, and expose only a count ahead plus validated model results/customer-safe errors. Worker-controlled queue data never supplies a filesystem path. Persistent slice identities include pipeline/profile version, stored model format, canonical file hash, and canonical slice settings, preventing cross-format/polyglot or stale-profile reuse. Each slice retry receives a fresh attempt UUID/job ID; all claims and writes conditionally match the live generation. The worker reloads authoritative rows and verifies job UUIDs, model/hash/settings/result binding before use. |
| Native slicer | Every production job receives a size/hash-verified private copy and distinct UID/GID. Orca runs with a minimal explicit credential-free environment, no capabilities, `no-new-privileges`, no internet route, a per-job timeout, and escaped-process reaping by UID. Telegram alerting stays in the web service, which observes the Redis heartbeat; neither provider credentials nor an edge network are added to the worker. CPU, RAM, and PID limits are aggregate worker-container limits shared by concurrent slicers, not per-Orca resource quotas. Trusted reads reject symlinks and bound result JSON, output archive, metadata, strings, and numeric statistics. A reaping failure terminates the worker before a UID can be reused. The source-run CI funnel may substitute fixed synthetic measurements only after the same no-follow size/hash staging; the process-level flag is not queue-controlled, is refused outside explicit development/test mode, is absent from Compose, and must use disposable stores. |
| Operator alerts | Unexpected checkout exceptions, checkout capacity 503s, quotation-PDF failures, the Shiprocket daily cap, ingest queue saturation, and a worker heartbeat missing for more than one minute send static, PII-free Telegram alerts. Per-kind Redis claims cap sends to one per 15 minutes across web replicas and carry a suppressed count into the next message; an in-process fallback retains the cap during Redis outages. Alerts are fire-and-forget and never change customer-path results. |
| Checkout, retention, and cleanup | Checkout atomically marks each model single-use; per-IP limits plus a 200/day global circuit breaker bound permanent rows/PDFs/notifications. PDFs are capped at 20 MiB and use a short Redis lease around the final free-space check and exclusive write. Terminal quotation states are immutable through conditional state transitions, allowing cleanup to establish a stable lifetime boundary. Paginated cleanup handles unattached uploads, purges terminal quotations' model files after 30 days, and deletes terminal quotation rows with their customer fields, PDFs, and now-orphaned models on the daily sweep after a configured threshold capped at 90 days. Non-terminal quotations are never aged out. Orphaned derived files are reconciled using server-derived paths. |
| Database and Redis | PostgreSQL and authenticated Redis are internal-only. A short-lived migration service holds the schema-owner credential. It provisions distinct web and worker roles, removes public database/schema creation, and reapplies explicit grants; only the worker may insert `UploadedModel`, while the web role has column-scoped `SliceResult` lifecycle/identity writes and cannot write slicer measurements or raw metadata. Long-running services never receive the owner URL. Prisma production query logging is disabled. |
...
- Geometry inspection is bounded, globally serialized, and off the public web
  event loop, closing the former advisory-lock overlap and web-availability
  risk. It remains synchronous inside the trusted worker orchestrator, so a
  parser crash/OOM can disrupt that container. A short-lived credential-free
  child process or dedicated ingest VM remains the stronger isolation boundary.
...
- Terminal quotation rows, their contact/address fields, and PDFs are deleted
  automatically by the daily sweep after the configured threshold (90 days by
  default and at most), but non-terminal quotations do not age out; the
  operator must complete, deliver, or cancel each order to start that stable
  window.
  Checkout, contact, and shipping workflows intentionally send relevant PII to
  WhatsApp, Telegram, Resend, and Shiprocket when configured. The local sweep
  cannot delete copies held by those providers or retained backups, so enforce
  suitable vendor and backup retention separately.
- Web has broad DML rights over business tables except upload creation; the
  worker alone inserts `UploadedModel` and has selected read/update/delete
  grants. Slice measurement/raw-metadata columns are an
